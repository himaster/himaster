...
async function performClone({ repository, repoPath, repoUrl }) {
  if (fs.existsSync(repoPath)) {
    const answer = process.env.FORCE_REMOVE || await ask(
...
async function performGitOperationsStochastic({
  ...
  const repoPath = `${process.cwd()}/repos/${repository}`;
...
async function main() {
  const mode = process.env.COMMIT_MODE || await ask("Enter the commit mode (random, fix): ");
  const username = process.env.GITHUB_USERNAME || await ask("Enter your GitHub username: ");
  const accessToken = process.env.GITHUB_TOKEN || await ask("Enter your GitHub access token: ");
  const repository = process.env.GITHUB_REPO || await ask("Enter your GitHub repository name: ");
  const startDate = process.env.START_DATE && moment().format('YYYY-MM-DD') || await ask("Enter start date (YYYY-MM-DD): ");
  const endDate = process.env.END_DATE && moment().format('YYYY-MM-DD') || await ask("Enter end date (YYYY-MM-DD): ");
  if (mode === "fix") {
  ...
  if (mode !== "fix") {
    ...
    const maxCommits = process.env.MAX_COMMIT_NUMBER || await ask("Enter the maximum number of commits: ");
    for (var day of weekDays) {
      const ratio = parseFloat(
        process.env['RATIO_' + day] || await ask("Enter the " + day + " ratio to max commits (0 to 1):")
      );
      const appearProbability = parseFloat(
        process.env['PROBABILITY_' + day] || await ask("Enter the " + day + " appear probability (0 to 1): ")
      );
      ratios[day] = ratio;
